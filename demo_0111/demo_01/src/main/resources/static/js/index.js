...
function ValueChange(value,index){
    ...
    //alert(${"#res_resrult"}[0].value);
	 //alert($("#res_select").find("option:selected").val());
    //alert($("#res_select").find("option:selected").val());
	if($(value).val()==""){
        alert('不能为空！');
        return false;
    ...
    $.ajax({
		...
		data: {/*index:index.toString()+"&"+*/index:$(value).attr("id"),name:$(value).val(),res_select:$("#res_select").find("option:selected").val()}  ,
    	dataType:"json",
        ...
        success: function(msg){
			...
			if(msg){
                alert("该"+$("#res_select").find("option:selected").val() +$(value).attr("id")+":"+$(value).val()+" 已经存在");
...
function changetoannotion(){
    $("#form_login").fadeIn();
    //$("#nouser").innerHTML = map.put("msg",'&nbsp');
}
